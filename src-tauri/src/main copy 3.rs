...
async fn main() {
    let use_postgres = false; // Set to true to use PostgreSQL, false to use SQLite
      ...
      let pool: Box<dyn std::any::Any> = if use_postgres {
        ...
        Box::new(pg_pool) as Box<dyn std::any::Any>
    } else {
        ...
        Box::new(sqlite_pool) as Box<dyn std::any::Any>
    };
    ...
    tauri::Builder::default()
        .manage(PgPoolWrapper { pool: pool.clone() })
