...
use db_connections::{establish_database_connection,read_active_db,
    create_new, 
    read_file_contents, 
    read_specific_line, 
    read_file_content, write_to_file, DbPool};
...
async fn main() {
    ...
    match read_specific_line( 1) {
        Ok(line) => println!("Line 3: {}", line),
        Err(e) => eprintln!("Error: {}", e),
    }

    // let create_new = create_new();
    // println!("Create new: {:?}", create_new);
    // let read_file_contents = read_file_contents();
    // println!(" new contents: {:?}", read_file_contents);
